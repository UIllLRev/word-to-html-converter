...
.post p {overflow: visible;}";
    ...
    public static class WmlToHtmlConverter
    {
        ...
        private static object ConvertToHtmlFootnoteTransform(WordprocessingDocument wordDoc,
           WmlToHtmlConverterSettings settings, XNode node,
           bool suppressTrailingWhiteSpace,
           decimal currentMarginLeft,
           string footnoteId = null)
        {
            ...
            if (element.Name == W.p)
            {
                return ProcessFootnoteParagraph(wordDoc, settings, element, suppressTrailingWhiteSpace, currentMarginLeft, footnoteId);
            }
            ...
            if (element.Name == W.r)
            {
                return ConvertRun(wordDoc, settings, element, footnoteId);
            }
        ...
        // the element (e.g., h:h2) created from the w:p element having the (first)
        // style separator (i.e., a w:specVanish element).
        private static object ProcessFootnoteParagraph(WordprocessingDocument wordDoc, WmlToHtmlConverterSettings settings,
            XElement element, bool suppressTrailingWhiteSpace, decimal currentMarginLeft, string footnoteId)
        {
            // Ignore this paragraph if the previous paragraph has a style separator.
            // We have already transformed this one together with the previous one.
            var previousParagraph = element.ElementsBeforeSelf(W.p).LastOrDefault();
            ...
            var elementName = GetParagraphElementName(element, wordDoc);
            var isBidi = IsBidi(element);
            var paragraph = (XElement)ConvertFootnoteParagraph(wordDoc, settings, element, elementName,
        ...
        private static object ConvertFootnoteParagraph(WordprocessingDocument wordDoc, WmlToHtmlConverterSettings settings,
            XElement paragraph, XName elementName, bool suppressTrailingWhiteSpace, decimal currentMarginLeft, bool isBidi, string footnoteId)
        {
            var style = DefineParagraphStyle(paragraph, elementName, suppressTrailingWhiteSpace, currentMarginLeft, isBidi);
            var firstMark = isBidi ? new XEntity("#x200f") : null;
            ...
            var paraElement = new XElement(elementName,
                firstMark,
                txElementsPrecedingTab,
                ConvertFootnoteContentThatCanContainFields(wordDoc, settings, elementsSucceedingTab, footnoteId));
            paraElement.AddAnnotation(style);
        ...
        private static object ConvertRun(WordprocessingDocument wordDoc, WmlToHtmlConverterSettings settings, XElement run, string footnoteId = null)
        {
            var rPr = run.Element(W.rPr);
            if (rPr == null)
                return run.Elements().Select(e => ConvertToHtmlTransform(wordDoc, settings, e, false, 0m, footnoteId));
        ...
        private static object ConvertFootnoteContentThatCanContainFields(WordprocessingDocument wordDoc, WmlToHtmlConverterSettings settings,
            IEnumerable<XElement> elements, string footnoteId = null)
        {
            var grouped = elements
                .GroupAdjacent(e =>
                {
            ...
            var txformed = grouped
                .Select(g =>
                {
                    var key = g.Key;
                    if (key == null)
                        return (object)g.Select(n => ConvertToHtmlFootnoteTransform(wordDoc, settings, n, false, 0m, footnoteId));
                    ...
                    var parsed = FieldRetriever.ParseField(instrText);
                    if (parsed.FieldType != "HYPERLINK")
                        return g.Select(n => ConvertToHtmlFootnoteTransform(wordDoc, settings, n, false, 0m));
